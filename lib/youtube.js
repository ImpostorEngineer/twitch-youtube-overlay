...
export async function fetchPlaylistVideos(playlistIdInput) {
  const apiKey = getRequiredEnv("YOUTUBE_API_KEY");
  const playlistId = playlistIdInput || getRequiredEnv("YOUTUBE_PLAYLIST_ID");
  const videos = [];
  let pageToken = "";
...
export function extractPlaylistId(input) {
  ...
  if (YOUTUBE_PLAYLIST_ID_REGEX.test(trimmed)) {
    return trimmed;
  }
  ...
  try {
    ...
    if (YOUTUBE_PLAYLIST_ID_REGEX.test(playlistId || "")) {
      return playlistId;
    }
