...
function canControlPlayback(tags) {
...
async function resolveRequestedVideo(input) {
  ...
  if (directVideoId) {
    return fetchVideoDetails(directVideoId);
  }

  return searchYoutubeVideos(input);
}

function getRoomIdFromChannel(channel) {
  const normalizedChannel = channel.replace(/^#/, '').toLowerCase();
  return getRoom(normalizedChannel)?.id || null;
}

async function handleSongRequest(roomId, channel, client, userName, query) {
  if (!query) {
    replyIfPossible(client, channel, `@${userName} use !sr <search or youtube url>`);
    return;
  }

  if (!canUserRequest(roomId, userName)) {
  ...
  const result = addToQueue(roomId, video, {
...
async function handleCommand(channel, client, tags, message) {
  ...
  const roomId = getRoomIdFromChannel(channel);
  ...
  if (command === '!sr') {
    await handleSongRequest(roomId, channel, client, userName, query);
  }

  if (command === '!skip') {
    if (!canControlPlayback(tags)) {
      replyIfPossible(client, channel, `@${userName} only the broadcaster or a moderator can use !skip.`);
      return;
    }

    skip(roomId);
    const current = getCurrent(roomId);
...
export function ensureTwitchService() {
  ...
  state.starting = (async () => {
    try {
      ...
      const channels = getRooms().map((room) => room.id);
      const clientOptions = {
        channels,
      };
      ...
      client.on('connected', () => {
        ...
        console.log(`Connected to Twitch channels ${channels.join(', ')} in ${mode} mode`);
      });
